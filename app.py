...
if uploaded_pdf:
    ...
    chunks = create_chunks(text, chunk_size=1000, overlap=300)
    ...
    if question:
        ...
        st.text_area( "", context, height=300)

        answer = generate_answer( question, context, tokenizer,llm_model)
        ...
        for rank, idx in enumerate(indices[0]):
            ...
            st.write(f"Distance: {distances[0][rank]:.4f}" )
            st.write(chunks[idx])
            st.divider()
